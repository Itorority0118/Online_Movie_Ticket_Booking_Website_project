...
public class SeatServlet extends HttpServlet {
	...
	// helper forward
	private void forward(HttpServletRequest request, HttpServletResponse response, boolean isAjax, String jsp)
			throws ServletException, IOException {
		if (isAjax)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("generate".equals(action)) {
			...
			if (fromRow < 'A' || fromRow > 'Z') {
				errors.put("fromRow", "From Row phải đặt theo thứ tự bảng chữ cái");
			}

			if (toRow < 'A' || toRow > 'Z') {
				errors.put("toRow", "To Row phải đặt theo thứ tự bảng chữ cái");
			}

			if (fromRow > toRow) {
				errors.put("rowOrder", "From Row phải đặt theo thứ tự bảng chữ cái");
			}

			if (seatPerRow <= 0) {
				errors.put("seatPerRow", "Seats per row phải lớn hơn 0");
			}

			if (seatType == null || seatType.isEmpty()) {
				errors.put("seatType", "Seat Type chưa chọn");
			}
			...
			if (!errors.isEmpty()) {
				...
				request.setAttribute("roomId", roomId);
				request.setAttribute("showtimeId", showtimeIdStr);
				request.getRequestDispatcher("/admin/dashboard.jsp?page=seat-generate.jsp").forward(request, response);
				return;
			}
		...
		if (seatRow == null || seatRow.isEmpty())
			errors.put("seatRow", "Seat Row is required");
		if (seatColStr == null || seatColStr.isEmpty())
			errors.put("seatCol", "Seat Column is required");
		if (seatType == null || seatType.isEmpty())
		...
		try {
			if (seatColStr != null && !seatColStr.isEmpty()) {
				...
				if (seatCol <= 0)
					errors.put("seatCol", "Seat Column must be positive");
			}
		...
		if (!errors.isEmpty()) {
			request.setAttribute("errors", errors);
			request.setAttribute("seat", seat);
			request.getRequestDispatcher("/admin/dashboard.jsp?page=seat-form.jsp").forward(request, response);
			return;
		}
